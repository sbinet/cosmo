...
func benchmarkLambdaCDMDistanceFlat(f func(float64) float64, b *testing.B) {
	z := 1.0
	for i := 0; i < b.N; i++ {
		f(z)
	}
...
//   for an Omega_K > 0 cosmology
func benchmarkLambdaCDMDistancePositiveOk0(f func(float64) float64, b *testing.B) {
	z := 1.0
	for i := 0; i < b.N; i++ {
		f(z)
	}
...
func benchmarkLambdaCDMDistanceOM(f func(float64) float64, b *testing.B) {
	z := 1.0
	for i := 0; i < b.N; i++ {
		f(z)
	}
...
func BenchmarkLambdaCDMComovingDistanceOM(b *testing.B) {
	...
	benchmarkLambdaCDMDistanceOM(cos.ComovingDistance, b)
}

func BenchmarkLambdaCDMLookbackTimeOM(b *testing.B) {
	cos := LambdaCDM{H0: 70, Om0: 0.27, Ol0: 0.}
	benchmarkLambdaCDMDistanceOM(cos.LookbackTime, b)
}

func BenchmarkLambdaCDMComovingDistanceFlat(b *testing.B) {
	cos := LambdaCDM{H0: 70, Om0: 0.3, Ol0: 0.7}
	benchmarkLambdaCDMDistanceFlat(cos.ComovingDistance, b)
}

func BenchmarkLambdaCDMComovingDistancePositiveOk0(b *testing.B) {
	cos := LambdaCDM{H0: 70, Om0: 0.3, Ol0: 0.9}
	benchmarkLambdaCDMDistancePositiveOk0(cos.ComovingDistance, b)
}
